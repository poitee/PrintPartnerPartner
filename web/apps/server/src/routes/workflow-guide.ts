...
Print Partner helps you manage **layered STL kits** — a base repo plus add-on repos — through a three-step pipeline: **Sources → Build → Review**. Plan management (create, rename, duplicate, delete) lives outside that pipeline — use the header **Create build** button, the **Manage builds** panel on Build, or the **Builds** page in the sidebar.
...
The active plan is shared across Build and Review. Switch plans in the header picker or **Manage builds** without reconfiguring each step separately.
...
- **Role filament colors** — assign a color per role (primary, accent, clear, opaque); previews update automatically on Build and Review.
...
Confirm a **validation summary** grouped by role and filament. Browse the grouped parts sheet with **3D STL previews**, edit quantities, remove or restore parts, and track **per-unit print progress** (Edit | Print view). Filter by print status, source, folder, role, or filament. **Export STLs**, **Export missing STLs**, **Export checklist** HTML, and **Print** the sheet from the header. On-scroll thumbnails render client-side for each part.
...
- **Progress widget** — the sidebar shows a first-run checklist until you complete Sources through Review once; it then hides automatically.
...
- **Spoolman** — connect in Settings → Integrations for live filament inventory on Build and spool weights in Review. See the Spoolman integration doc in the repo.
