...
The client sends the chosen Build ID, the proposed `part_id + unit_index` pairs with the observed Object name on each unit, and unmatched names with the G-code. Once the host accepts the upload, the server creates a checkoff link with an immutable Build binding, host and printer IDs, local and remote filenames, upload job ID, proposed units, and unmatched names. [client upload payload](../../../../web/apps/web/src/components/export/PrinterSendPanel.tsx#L366-L401) [upload result and link creation](../../../../web/apps/server/src/services/printer-upload-job.ts#L128-L185)

Each mapped unit keeps the Object name that produced it. The link also stores names that failed to map. Verify identity remains `part_id + unit_index`; the Object name is the audit trail from the sliced label to that Required unit. Unattributed prints still retain the raw G-code object-name list and candidate filenames separately. [checkoff unit fields](../../../../web/packages/contracts/src/index.ts#L207-L213) [link storage](../../../../web/apps/server/src/services/printer-checkoff-store.ts#L207-L247) [unattributed print fields](../../../../web/apps/server/src/services/unattributed-print-store.ts#L4-L25)
...
- Linked records keep the Object name that mapped onto each unit, plus unmatched labels. Verify still keys units by `part_id + unit_index`. [linked record shape](../../../../web/packages/contracts/src/index.ts#L207-L213)
