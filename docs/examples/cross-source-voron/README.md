...
Minimal `print-partner.manifest.yaml` snippets for a Voron base + Stealthburner + umbilical stack. Copy into each synced repo root, or define import rules on **Library** and file picks on **Sources**.
...
After saving manifests to repo roots: sync on **Library** → attach layers on **Sources** → **Apply** on **Plan** → **Production**.
