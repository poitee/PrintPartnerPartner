...
| **Printer** | Each Plate carries the Printer ID, name, model, bed size, and margin captured when that Plate revision was published. Production does not choose or reroute a Printer. |
...
2. On Production, assign every Required unit to a Printer. Source-layer and role
...
1. Record the Plate revision shown on Production.
...
6. Confirm Plate placement matches the saved Production layout.
